...
with CascadeWrapperBase() as cascade:
    # Build a Path identifier for a page asset
...
The `read` operation represents the response shape that most other "fetch" operations follow, such as `readAudits`, `readAccessRights`, and `readWorkflowSettings`. Each of these operations returns a structured object specifically tailored to the requested resource.
...
Mutating operations like `delete`, `copy`, `move`, `publish`, `checkIn`, and `editAccessRights` return confirmation status only rather than the modified asset data. Callers should not expect to receive asset payloads back from these calls.
...
if isinstance(result, CascadeError):
    ...
    # Iterate through the elements found in the search list result
    for element in result.flat:
        print(element)
```

The `search` operation requires a typed payload object and does not support bare identifier shortcuts. Other operations that follow this same pattern include `readAudits` (which takes `auditParameters`), `editWorkflowSettings`, and others that rely on structured input payloads.
