...
with CascadeWrapperBase() as cascade:
    # Define the asset path using the Path TypedDict
    ...
    # Check for API-level failures before accessing properties
...
```python
from cascade_cms import CascadeWrapperBase
from cascade_cms.cmstypes import Path, deleteParameters, SearchInformation

with CascadeWrapperBase() as cascade:
    path_id = Path(path="index", siteName="default", asset_type="page")
    delete_id = Path(path="old-page", siteName="default", asset_type="page")
    delete_params = deleteParameters(doWorkflow=False, destinations=[], unpublish=True)
    ...
    cascade.operations.delete(delete_id, payload=delete_params)
