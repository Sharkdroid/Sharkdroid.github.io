...
with CascadeWrapperBase("https://cascade.example.com", "username", "password") as cascade:
    ...
    path = Path(asset_type="page", path="/index", siteName="Default Site")
    
    # 2. Queue the read operation
    cascade.operations.read(path)
    
    # 3. Execute all queued requests
    results = cascade.submit_requests()
    result = results[0]
    
    # 4. Check for errors before accessing asset fields
...
with CascadeWrapperBase("https://cascade.example.com", "username", "password") as cascade:
    ...
    path = Path(asset_type="page", path="/old-page", siteName="Default Site")
    
    # 2. Prepare delete parameters payload
    ...
    # 4. Execute requests and check result
    results = cascade.submit_requests()
    result = results[0]
...
with CascadeWrapperBase("https://cascade.example.com", "username", "password") as cascade:
    ...
    search_info = SearchInformation(siteName="Default Site", searchTerms="report")
...
with CascadeWrapperBase("https://cascade.example.com", "username", "password") as cascade:
    path = Path(asset_type="page", path="/index", siteName="Default Site")
    delete_path = Path(asset_type="page", path="/old-page", siteName="Default Site")
    search_info = SearchInformation(siteName="Default Site", searchTerms="report")

    # Queue multiple independent chains
    cascade.operations.read(path)
    cascade.operations.delete(delete_path, payload=deleteParameters(doWorkflow=False, destinations=[], unpublish=True))
    cascade.operations.search(search_info)
