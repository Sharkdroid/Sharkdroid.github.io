...
if isinstance(result, CascadeError):
    ...
    for msg in result.flat:
        if isinstance(msg, Message):
            # Mark message as read
            msg.marked = "read"
            cascade.operations.markMessage(message=msg)
            
            # Or delete the message
            # cascade.operations.deleteMessage(message=msg)

    action_result = cascade.submit_requests()
    if isinstance(action_result, CascadeError):
        print(f"Failed to update message: {action_result.message}")
```
...
if isinstance(prefs, CascadeError):
    ...
    pref_payload = preference(name="dateFormat", value="YYYY-MM-DD")
    cascade.operations.editPreference(payload=pref_payload)
